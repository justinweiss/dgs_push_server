...
class ApnsDevicesControllerTest < ActionController::TestCase
  ...
  test "If we get a new token without a user or a device id, create them both" do
    assert_difference "ApnsDevice.count", 1 do
      assert_difference "Player.count", 1 do
        post :create, player_id: 1000, device: { device_token: "new_device_token" }
    ...
    assert_difference "ApnsDevice.count", 1 do
      assert_difference "Player.count", 0 do
        assert_difference "@main_player.reload.apns_devices.count", 1 do
          post :create, player_id: @main_player.dgs_user_id, device: { device_token: "new_device_token" }
          assert_response :success
        end
      end
    end
  ...
  test "If we already have this device token, return the existing token and don't create anything" do
    ...
    assert_difference "ApnsDevice.count", 0 do
      assert_difference "Player.count", 0 do
        post :create, player_id: @main_player.dgs_user_id, device: { device_token: @main_device.device_token }
        assert_response :success
  ...
  test "If we already have this device token, but with a different player, re-associate the device token with the new player and don't create a new device" do
    assert_difference "ApnsDevice.count", 0 do
      assert_difference "Player.count", 1 do
        assert_difference "@main_player.reload.apns_devices.count", -1 do
          post :create, player_id: 10, device: { device_token: @main_device.device_token }
          assert_response :success
  ...
  test "If we match a token but the device id doesn't match, return the matching device id" do
    put :update, id: 100, player_id: @main_player.dgs_user_id, device: { device_token: @main_device.device_token }
    assert_response :success
    assert_equal @main_device.id, assigns(:device).id
  end
  ...
  test "If we match a token but the user doesn't match, and is new, create the user" do
    assert_difference "Player.count", 1 do
      put :update, id: @main_device.id, player_id: 4, device: { device_token: @main_device.device_token }
    end
    assert_response :success
    ...
    assert_difference "Player.count", 0 do
      assert_difference "ApnsDevice.count", 0 do
        assert_difference "players(:user_without_tokens).apns_devices.count", 1 do
          put :update, id: @main_device.id, player_id: players(:user_without_tokens).dgs_user_id, device: { device_token: @main_device.device_token }
        end
      end
    end
    assert_response :success
  ...
  test "If we pass in a new token for a device id, update the token" do
    put :update, id: @main_device.id, player_id: @main_player.dgs_user_id, device: { device_token: "new_device_token" }
  ...
  test "If we pass in completely new data, create all the necessary records" do
    assert_difference "Player.count", 1 do
      assert_difference "ApnsDevice.count", 1 do
        put :update, id: 123456, player_id: 1000, device: { device_token: "new_device_token" }
        assert_response :success
      end
    end
  ...
  test "Render a 404 if we don't have an app passed in for updating" do
    ...
    assert_raises(ActionController::RoutingError) do
      put :update, id: 1000, player_id: @main_player.dgs_user_id, device: { device_token: "main_device_token" }
    end
  end
  ...
  test "Delete can delete a device by device token" do
    assert_difference "ApnsDevice.count", -1 do
      delete :destroy, player_id: @main_player.dgs_user_id, id: 1234, device: {device_token: @main_device.device_token}
      assert_response :success
    end
  end
  ...
  test "The token should get touched even if there are no changes" do
    ...
    Time.stub :now, 1.minute.from_now do
      put :update, id: @main_device.id, player_id: @main_player.dgs_user_id, device: { device_token: @main_device.device_token }
